...
if ($contentId > 0) {
    ...
    try {
        $stmt = $pdo->prepare("SELECT * FROM konten WHERE id = ? AND kategori = 'berita'");
        $stmt->execute([$contentId]);
        $content = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if (!$content) {
            // Jika konten tidak ditemukan, redirect ke halaman berita
            header('Location: berita.php');
            exit;
        }
        
        // Ambil konten terkait
        $stmt = $pdo->prepare("SELECT * FROM konten WHERE kategori = 'berita' AND id != ? ORDER BY created_at DESC LIMIT 3");
        $stmt->execute([$contentId]);
...
<head>
    ...
    <title>Berita - SMA Negeri 72 Jakarta</title>
                    ...
                    <article class="card shadow-sm">
                        ...
                        <div class="card-body">
                            ...
                            <div class="content-meta mb-4">
                                <span class="badge bg-primary"><?php echo ucfirst(htmlspecialchars($content['kategori'])); ?></span>
                            ...
                            <?php else: ?>
                                <?php foreach ($relatedContent as $related): ?>
                                    <div class="mb-3 pb-3 border-bottom">
                                        <h6><a href="berita.php?id=<?php echo $related['id']; ?>" class="text-decoration-none"><?php echo htmlspecialchars($related['judul']); ?></a></h6>
                ...
                <div class="col-12">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h1 class="display-6 mb-0">
                            <i class="fas fa-newspaper text-primary me-2"></i>Berita
                        </h1>
                        <?php if (function_exists('isLoggedIn') && isLoggedIn()): ?>
                            <a href="<?php echo (function_exists('isAdmin') && isAdmin()) ? '../admin/tambah_artikel.php?kategori=berita' : '../redaksi/tambah_artikel.php?kategori=berita'; ?>" class="btn btn-primary">
                            ...
                            <div class="card h-100 shadow-sm">
                                ...
                                <div class="card-body d-flex flex-column">
                                    ...
                                    <div class="d-flex justify-content-between">
                                        <a href="berita.php?id=<?php echo $content['id']; ?>" class="btn btn-primary">
                                            <i class="fas fa-book-reader me-1"></i> Baca Selengkapnya
                                        </a>
                                        <?php if (function_exists('isLoggedIn') && isLoggedIn()): ?>
                                            <a href="<?php echo (function_exists('isAdmin') && isAdmin()) ? '../admin/edit_artikel.php?id=' . $content['id'] : '../redaksi/edit_artikel.php?id=' . $content['id']; ?>" class="btn btn-outline-primary">
